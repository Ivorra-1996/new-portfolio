...
void main() {
  ...
  // Pupil con seguimiento del cursor
...
export default function EvilEye({
  ...
  useEffect(() => {
    ...
    // A diferencia del componente original (que sigue el mouse solo dentro
    // de su propio contenedor), acá el Ojo vive en un ícono chico del Header
    // pero tiene que "vigilar" al cursor-Anillo en toda la pantalla, así que
    // escuchamos el mousemove en window en vez de en el contenedor.
    function onMouseMove(e: MouseEvent) {
      const rect = container.getBoundingClientRect();
      const cx = rect.left + rect.width / 2;
      const cy = rect.top + rect.height / 2;
      mouse.tx = Math.max(-1, Math.min(1, (e.clientX - cx) / (window.innerWidth / 2)));
      mouse.ty = Math.max(-1, Math.min(1, -((e.clientY - cy) / (window.innerHeight / 2))));
    }

    window.addEventListener('mousemove', onMouseMove);

    // eslint-disable-next-line prefer-const -- se asigna después de declararse, la usa `resize` antes de existir
    let program: Program;
