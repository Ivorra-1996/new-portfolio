...
/**
 * Fondo shader "MoltenMetal" (port a TypeScript vanilla + ogl del componente
 * React original de react-bits — este proyecto no usa React). Crea su propio
 * <canvas> dentro de `container` y lo anima con requestAnimationFrame,
 * pausándose cuando la sección no está visible o la pestaña está en segundo
 * plano. Devuelve una función de limpieza.
 */
export function initMoltenMetal(
  container: HTMLElement,
  options: MoltenMetalOptions = {},
): () => void {
  const {
    color1 = "#0a0a0a",
    color2 = "#4a5a63",
  ...
  function tryStart() {
    if (isVisible && isPageVisible && raf === 0 && !prefersReducedMotion) {
      raf = requestAnimationFrame(loop);
    }
  }
  function tryStop() {
